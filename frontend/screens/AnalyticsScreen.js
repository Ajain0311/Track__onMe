...
export default function AnalyticsScreen() {
  ...
  const loadData = useCallback(async () => {
    setError(null);
    try {
      // Fetch server data to keep in sync; result not rendered directly
              ...
              return (
                <View key={day.date} style={styles.barContainer}>
                  <View style={styles.barWrapper}>
                    <View style={[styles.bar, { height: barHeight, backgroundColor: isToday ? g.mint : g.accentSoft }]} />
                  </View>
                  <Text style={[styles.barLabel, { color: isToday ? g.mint : g.textMuted }]}>{day.label}</Text>
                  <Text style={[styles.barValue, { color: g.textDim }]}>{day.hours > 0 ? `${Math.round(day.hours)}h` : '-'}</Text>
                </View>
              );
            })}
          </View>
...
const styles = StyleSheet.create({
  ...
  summaryCard: { flex: 1, borderRadius: 16, padding: 16, borderWidth: 1, alignItems: 'center' },
  summaryValue: { fontSize: 20, fontWeight: '900', fontVariant: ['tabular-nums'] },
  summaryLabel: { fontSize: 12, marginTop: 4, fontWeight: '600' },
  chartCard: { borderRadius: 20, padding: 20, marginBottom: 16, borderWidth: 1 },
  ...
  barWrapper: { width: 24, height: 120, justifyContent: 'flex-end', borderRadius: 4, overflow: 'hidden' },
